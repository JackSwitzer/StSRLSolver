...
7. DOCS AND AUDIT-RESULT REFRESH — the docs tree carries legacy content that predates the verification sweep and now contradicts the code: `docs/work_units/audit-reports/` (April 2026 imports, explicitly labeled a pre-sweep "fix quarry"), stale sections of `docs/work_units/parity-deviations-register.md`, vault notes, and any README/doc describing superseded flows or dead code you removed in area 6. For each stale doc: if it still has reference value, move it to `archive/` with a dated note; if its content is simply wrong about the current engine, rewrite the affected sections; if it duplicates the ledger/FINDINGS, delete the duplication and point to the canonical file. `docs/goal/` (GOAL, FINDINGS, UNITS, ledger, masks, BENCHMARKS, TOOLING) is canonical — refresh other docs to agree with it, never the reverse.
...
In your final chat answer, return ONLY: the top-10 table, counts per band, and a diffstat summary of the cleanup commits (areas 6-7). Highest-value findings first.
</structured_output_contract>
...
- Areas 6-7 are execute-now: dead-code deletion and docs refresh are real changes on this branch, committed in small reviewable units (`cleanup(code): ...` / `cleanup(docs): ...`), separate from the register commit. Dead CODE is deleted outright — no `#[allow(dead_code)]`, no commenting out, no backwards-compat stubs. Superseded DOCS are archived to `archive/` with a dated note or rewritten, never silently deleted. If a deletion candidate is load-bearing for behavior parity, it is not dead — leave it and register the confusion as a finding instead.
