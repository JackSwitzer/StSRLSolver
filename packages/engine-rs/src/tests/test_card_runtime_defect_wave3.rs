...
fn test_card_runtime_defect_wave3_registry_exports_surface_x_cost_and_exhaust_hints() {
    ...
    assert_eq!(
        reboot.declared_effect_count, 1,
    ...
    let reboot_def = crate::cards::global_registry()
        .get("Reboot")
        .expect("Reboot");
    assert_eq!(
    ...
    let fission_def = crate::cards::global_registry()
        .get("Fission")
        .expect("Fission");
    assert_eq!(
    ...
    let fission_plus_def = crate::cards::global_registry()
        .get("Fission+")
        .expect("Fission+");
    assert_eq!(
...
fn multi_cast_preserves_energy_without_an_orb_and_chemical_x_repeats_front_orb() {
    let mut no_orb = engine_without_start(Vec::new(), vec![enemy_no_intent("JawWorm", 80, 80)], 3);
    ...
    let mut chemical =
        engine_without_start(Vec::new(), vec![enemy_no_intent("JawWorm", 80, 80)], 0);
    ...
    assert_eq!(
        chemical.state.orb_slots.front_orb_type(),
        OrbType::Lightning
    );
}
