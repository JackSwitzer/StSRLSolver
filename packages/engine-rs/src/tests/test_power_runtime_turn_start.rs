...
fn hello_world_hook_rolls_every_stack_and_spills_past_hand_limit() {
    ...
    let expected: Vec<&str> = (0..3)
        .map(|_| {
            DEFECT_COMMON_POOL[oracle.random_int((DEFECT_COMMON_POOL.len() - 1) as i32) as usize]
        })
        .collect();
    ...
    assert_eq!(
        engine.card_registry.card_name(engine.state.hand[9].def_id),
        expected[0]
    );
    ...
    assert_eq!(
        engine
            .card_registry
            .card_name(engine.state.discard_pile[1].def_id),
        expected[2]
    );
...
fn creative_ai_hook_rolls_every_stack_and_spills_past_hand_limit() {
    ...
    let expected: Vec<&str> = (0..3)
        .map(|_| {
            DEFECT_POWER_POOL[oracle.random_int((DEFECT_POWER_POOL.len() - 1) as i32) as usize]
        })
        .collect();
...
fn mayhem_hook_autoplays_top_draw_cards_for_free_without_exhausting() {
    ...
    assert_eq!(
        discard_names,
        vec!["Bash".to_string(), "Defend".to_string()]
    );
