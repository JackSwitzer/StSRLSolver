...
fn dead_cleanup_wave1_relic_bundle_is_engine_path_authoritative() {
    ...
    orange_engine.state.hand = make_deck(&["Strike", "Defend", "Inflame"]);
    orange_engine.state.player.set_status(sid::WEAKENED, 2);
    orange_engine.state.player.set_status(sid::VULNERABLE, 2);
    assert!(play_on_enemy(&mut orange_engine, "Strike", 0));
    assert!(play_self(&mut orange_engine, "Defend"));
    assert_eq!(
    ...
    nib_engine.state.hand = make_deck_n("Strike", 10);
    for _ in 0..9 {
        assert!(play_on_enemy(&mut nib_engine, "Strike", 0));
    }
