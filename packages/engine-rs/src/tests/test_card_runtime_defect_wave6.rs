...
fn defect_wave6_registry_exports_honest_runtime_surface() {
    let strike = global_registry()
        .get("Strike+")
        .expect("Strike_B+ should exist");
    ...
    let defend = global_registry()
        .get("Defend+")
        .expect("Defend_B+ should exist");
    ...
    let self_repair = global_registry()
        .get("Self Repair+")
        .expect("Self Repair+ should exist");
    assert_eq!(
    ...
    let boot_sequence = global_registry()
        .get("BootSequence+")
    ...
    assert_eq!(
        static_discharge.runtime_traits(),
...
fn defect_wave6_strike_defend_leap_boot_sequence_and_reinforced_body_follow_engine_path() {
    ...
    let mut reinforced_body =
        engine_without_start(Vec::new(), vec![enemy_no_intent("JawWorm", 20, 20)], 5);
...
fn defect_wave6_self_repair_machine_learning_and_static_discharge_install_runtime_statuses() {
    let mut self_repair =
        engine_without_start(Vec::new(), vec![enemy_no_intent("JawWorm", 6, 6)], 3);
    ...
    machine_learning.state.draw_pile =
        make_deck(&["Strike", "Defend", "Zap", "Dualcast", "Strike", "Defend"]);
    ...
    let mut static_discharge =
        engine_without_start(Vec::new(), vec![enemy("JawWorm", 40, 40, 1, 5, 1)], 3);
    ...
    assert_eq!(
        static_discharge.state.player.status(sid::STATIC_DISCHARGE),
        2
    );
    ...
    assert_eq!(
        static_discharge.state.orb_slots.slots[0].orb_type,
        OrbType::Lightning
    );
    assert_eq!(
        static_discharge.state.orb_slots.slots[1].orb_type,
        OrbType::Lightning
    );
}
...
fn static_discharge_triggers_before_tungsten_but_after_buffer() {
    ...
    let mut tungsten = engine_without_start(Vec::new(), vec![enemy("JawWorm", 40, 40, 1, 1, 1)], 3);
    ...
    assert_eq!(
        tungsten.state.orb_slots.slots[0].orb_type,
        OrbType::Lightning
    );

    let mut buffer = engine_without_start(Vec::new(), vec![enemy("JawWorm", 40, 40, 1, 1, 1)], 3);
...
fn static_discharge_full_slot_uses_focus_and_canonical_evokes_for_each_stack() {
    ...
    let mut engine = engine_without_start(Vec::new(), vec![enemy("JawWorm", 60, 60, 1, 5, 2)], 3);
    force_player_turn(&mut engine);
