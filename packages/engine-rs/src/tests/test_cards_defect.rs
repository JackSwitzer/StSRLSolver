...
mod defect_card_java_parity_tests {
    ...
    defect_test!(
        stack_snapshots_discard_size_and_modifies_the_combined_upgraded_block_once,
        {
            ...
            upgraded.state.discard_pile =
                make_deck(&["Strike", "Defend", "Zap", "Dualcast", "Leap"]);
    ...
    defect_test!(sweeping_beam_damages_all_enemies_before_its_single_draw, {
        ...
        let mut surviving =
            bare_engine(&[], vec![enemy("JawWorm", 20, 0), enemy("Cultist", 30, 0)]);
        ...
        let mut lethal = bare_engine(&[], vec![enemy("JawWorm", 9, 0), enemy("Cultist", 9, 0)]);
    ...
    defect_test!(
        hologram_matches_block_exhaust_and_discard_retrieval_edges,
        {
            ...
            let mut empty = bare_engine(&[], vec![enemy_no_intent("JawWorm", 40, 40)]);
            ...
            let mut singleton = bare_engine(&[], vec![enemy_no_intent("JawWorm", 40, 40)]);
            ...
            let mut multiple = bare_engine(&[], vec![enemy_no_intent("JawWorm", 40, 40)]);
    ...
    defect_test!(
        aggregate_variants_gain_truncated_energy_from_current_draw_pile,
        {
    ...
    defect_test!(steam_power_draws_before_adding_one_base_burn_to_discard, {
        ...
        assert_eq!(
            base.card_registry
                .card_name(base.state.discard_pile[0].def_id),
            "Burn"
        );
        assert_eq!(
            base.card_registry
                .card_name(base.state.discard_pile[1].def_id),
            "Steam Power"
        );
    ...
    defect_test!(
        skim_plus_spends_one_energy_draws_four_and_discards_normally,
        {
            ...
            engine.state.draw_pile = make_deck(&["Strike", "Defend", "Bash", "Inflame", "Zap"]);
            engine.state.discard_pile.clear();
    ...
    defect_test!(
        lock_on_source_applies_after_its_hit_and_amplifies_orb_thorns,
        {
    ...
    defect_test!(
        conserve_battery_plus_blocks_and_grants_one_energy_next_turn,
        {
            ...
            capped.state.player.set_status(sid::ENERGIZED_BLUE, 999);
    ...
    defect_test!(double_energy_doubles_energy_remaining_after_its_cost, {
        ...
        for (card_id, starting_energy, expected_energy) in
            [("Double Energy", 4, 6), ("Double Energy+", 4, 8)]
        {
            ...
            assert!(e
                ...
                .exhaust_pile
                .iter()
                .any(|c| { e.card_registry.card_name(c.def_id) == card_id }));
        }
    });

    defect_test!(
        fusion_channels_exactly_one_plasma_and_upgrade_only_lowers_cost,
        {
    ...
    defect_test!(
        go_for_the_eyes_weakens_exactly_the_four_attacking_intent_variants,
        {
            ...
            assert_eq!(
                non_attacking.state.enemies[0].entity.status(sid::WEAKENED),
                0
            );

            let mut artifact = bare_engine(&[], vec![enemy("JawWorm", 40, 0)]);
            artifact.state.enemies[0].intent = Intent::Attack {
                damage: 6,
                hits: 1,
                effects: 0,
            };
    ...
    defect_test!(hyperbeam_aoe_and_negative_focus_match_java_action_order, {
        ...
        let mut lethal = bare_engine(&[], vec![enemy_no_intent("JawWorm", 26, 26)]);
    ...
    defect_test!(
        impulse_triggers_every_orb_and_cables_with_card_random_ticks,
        {
            ...
            let mut engine = bare_engine(&[], vec![enemy_no_intent("JawWorm", 100, 100)]);
    ...
    defect_test!(
        meteor_strike_source_damage_precedes_three_plasma_channels,
        {
            ...
            assert!(nonlethal
                ...
                .slots
                .iter()
                .all(|orb| { orb.orb_type == OrbType::Plasma }));
    ...
    defect_test!(
        reprogram_plus_applies_artifact_blocked_focus_before_buffs,
        {
    ...
    defect_test!(sunder_refunds_only_when_its_kill_does_not_end_combat, {
        ...
        let mut mid_fight =
            bare_engine(&[], vec![enemy("JawWorm", 24, 0), enemy("Cultist", 40, 0)]);
    ...
    defect_test!(
        white_noise_uses_card_random_defect_power_pool_and_zeroes_turn_cost,
        {
    ...
    defect_test!(all_for_one_returns_zero_cost_cards_from_discard, {
        ...
        assert!(e
            ...
            .discard_pile
            .iter()
            .any(|card| { e.card_registry.card_name(card.def_id) == "Zap+" && card.cost == 1 }));
        assert!(e
            .state
            .discard_pile
            .iter()
            .any(|card| { e.card_registry.card_name(card.def_id) == "Turbo" }));
    ...
    defect_test!(
        amplify_variants_replay_power_cards_consume_charges_and_expire,
        {
    ...
    defect_test!(biased_cognition_bias_and_focus_loss_obey_artifact, {
        ...
        assert_eq!(
            blocked_tick.state.player.status(sid::BIASED_COG_FOCUS_LOSS),
            1
        );
    ...
    defect_test!(
        core_surge_variants_damage_then_grant_one_artifact_and_exhaust,
        {
            ...
            for (card_id, expected_hp) in [("Core Surge", 29), ("Core Surge+", 25)] {
                ...
                assert!(e
                    ...
                    .exhaust_pile
                    .iter()
                    .any(|card| { e.card_registry.card_name(card.def_id) == card_id }));
            }
        }
    );
    ...
    defect_test!(
        creative_ai_variants_stack_and_roll_one_defect_power_per_stack,
        {
    ...
    defect_test!(
        electrodynamics_installs_electro_before_channeling_lightning,
        {
    ...
    defect_test!(
        equilibrium_blocks_and_stacked_power_retains_for_each_round,
        {
            ...
            let mut engine = bare_engine(&[], vec![enemy_no_intent("JawWorm", 200, 200)]);
            ...
            engine
                .state
                .hand
                .retain(|card| engine.card_registry.card_name(card.def_id) == "Strike");
            engine.state.draw_pile.clear();
            engine.state.discard_pile.clear();
            end_turn(&mut engine);
            assert_eq!(hand_count(&engine, "Strike"), 1);
