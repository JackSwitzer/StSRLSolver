...
fn transmutation_moves_to_typed_generated_hand_surface() {
    ...
    let transmutation = registry
        .get("Transmutation")
        .expect("Transmutation should exist");
    let transmutation_plus = registry
        .get("Transmutation+")
        .expect("Transmutation+ should exist");
...
fn free_transmutation_plus_uses_current_x_and_chemical_x_then_spills_overflow() {
    ...
    engine.state.hand.push(
        engine
            .card_registry
            .make_card("Transmutation+")
            .set_free(true),
    );
    engine.state.relics.push("Chemical X".to_string());
    ...
    assert!(generated
        .iter()
        .all(|card| card.is_upgraded() && card.cost == 0));
