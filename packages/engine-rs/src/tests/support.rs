...
pub(crate) fn combat_state_with(deck: Vec<String>, enemies: Vec<EnemyCombatState>, energy: i32) -> CombatState {
...
pub(crate) fn engine_with(deck: Vec<String>, enemy_hp: i32, enemy_dmg: i32) -> CombatEngine {
...
pub(crate) fn engine_with_enemy_id(deck: Vec<String>, enemy_id: &str, enemy_hp: i32, enemy_dmg: i32) -> CombatEngine {
    engine_with_state(combat_state_with(
        deck,
...
pub(crate) fn engine_with_enemies(deck: Vec<String>, enemies: Vec<EnemyCombatState>, energy: i32) -> CombatEngine {
    engine_with_state(combat_state_with(deck, enemies, energy))
}

pub(crate) fn engine_without_start(deck: Vec<String>, enemies: Vec<EnemyCombatState>, energy: i32) -> CombatEngine {
...
pub(crate) fn ensure_in_hand(engine: &mut CombatEngine, card_id: &str) {
    if !engine.state.hand.iter().any(|card| card == card_id) {
...
pub(crate) fn play_card(engine: &mut CombatEngine, card_id: &str, target_idx: i32) -> bool {
    if let Some(idx) = engine.state.hand.iter().position(|c| c == card_id) {
...
pub(crate) fn hand_count(engine: &CombatEngine, exact_id: &str) -> usize {
    engine.state.hand.iter().filter(|card| card.as_str() == exact_id).count()
}

pub(crate) fn hand_prefix_count(engine: &CombatEngine, prefix: &str) -> usize {
    engine.state.hand.iter().filter(|card| card.starts_with(prefix)).count()
}

pub(crate) fn discard_prefix_count(engine: &CombatEngine, prefix: &str) -> usize {
    engine.state.discard_pile.iter().filter(|card| card.starts_with(prefix)).count()
}

pub(crate) fn exhaust_prefix_count(engine: &CombatEngine, prefix: &str) -> usize {
    engine.state.exhaust_pile.iter().filter(|card| card.starts_with(prefix)).count()
}

pub(crate) fn draw_prefix_count(engine: &CombatEngine, prefix: &str) -> usize {
    engine.state.draw_pile.iter().filter(|card| card.starts_with(prefix)).count()
}
