...
use crate::tests::support::{engine_with, ensure_in_hand, make_deck_n, play_on_enemy};
...
fn time_warp_runtime_ends_turn_before_double_tap_replay() {
    ...
    assert_eq!(
        engine.state.turn, 2,
        "turn should advance after Time Warp ends the turn"
    );
    ...
    assert!(
        time_warp_hook_seen,
        "time_warp hook should execute through the runtime"
    );
}
