...
mod ironclad_card_java_parity_tests {
    ...
    use crate::tests::support::{
        combat_state_with, discard_prefix_count, end_turn, enemy, enemy_no_intent, engine_with,
        engine_with_enemies, ensure_in_hand, exhaust_prefix_count, force_player_turn, hand_count,
        make_deck, make_deck_n, play_card, play_on_enemy, play_self, TEST_SEED,
    };
        ...
        ($name:ident, $id:literal, $up:literal,
         ...
         $ty:expr, $target:expr, $exhaust:expr) => {
            mod $name {
                ...
                #[test]
                fn base() {
                    assert_card($id, $bc, $bd, $bb, $bm, $ty, $target, $exhaust);
                }

                #[test]
                fn upgraded() {
                    assert_card($up, $uc, $ud, $ub, $um, $ty, $target, $exhaust);
                }
            }
        };
        ($name:ident, $id:literal, $up:literal,
         ...
         $ty:expr, $target:expr, $base_exhaust:expr, $up_exhaust:expr) => {
            mod $name {
                ...
                #[test]
                fn base() {
                    assert_card($id, $bc, $bd, $bb, $bm, $ty, $target, $base_exhaust);
                }

                #[test]
                fn upgraded() {
                    assert_card($up, $uc, $ud, $ub, $um, $ty, $target, $up_exhaust);
                }
            }
        };
    }
    ...
    fn anger_variants_damage_then_add_a_stat_equivalent_discard_copy() {
        ...
        let mut dynamic = engine_for(&[], &[], &[], vec![enemy("JawWorm", 50, 50, 1, 0, 1)], 0);
        ...
        assert!(copy.is_free());
        assert_ne!(
            copy.flags & crate::combat_types::CardInstance::FLAG_INNATE,
            0
        );
        assert_eq!(
            copy.flags & crate::combat_types::CardInstance::FLAG_RETAINED,
            0
        );
        assert_eq!(
            copy.flags & crate::combat_types::CardInstance::FLAG_PURGE,
            0
        );
        assert_eq!(
            copy.flags & crate::combat_types::CardInstance::FLAG_EXHAUST_ON_USE,
            0
        );
    }
    card_pair_test!(
    ...
    fn armaments_base_auto_or_selects_only_can_upgrade_cards_and_plus_upgrades_all() {
        ...
        assert!(none.state.hand.iter().any(|card| {
            none.card_registry.card_name(card.def_id) == "AscendersBane" && !card.is_upgraded()
        }));
        ...
        assert!(one
            ...
            .hand
            .iter()
            .any(|card| { one.card_registry.card_name(card.def_id) == "Strike+" }));
        ...
        assert!(many
            ...
            .hand
            .iter()
            .any(|card| { many.card_registry.card_name(card.def_id) == "Defend+" }));
        assert!(many
            .state
            .hand
            .iter()
            .any(|card| { many.card_registry.card_name(card.def_id) == "Strike" }));
        ...
        assert!(plus
            ...
            .hand
            .iter()
            .any(|card| { plus.card_registry.card_name(card.def_id) == "Defend+" }));
        assert_eq!(
            plus.state
                .hand
                .iter()
                .filter(|card| { plus.card_registry.card_name(card.def_id) == "Strike+" })
                .count(),
            2
        );
        assert!(plus.state.hand.iter().any(|card| {
            plus.card_registry.card_name(card.def_id) == "AscendersBane" && !card.is_upgraded()
        }));
    }
    card_pair_test!(
        body_slam,
    ...
    fn burning_pact_auto_resolves_small_hands_then_draws_and_choices_large_hands() {
        ...
        assert_eq!(
            choice.state.hand.len(),
            2,
            "draw waits for exhaust selection"
        );
    ...
    fn evolve_draws_its_power_amount_after_each_status_draw() {
        ...
        assert_eq!(
            non_status
                .card_registry
                .card_name(non_status.state.hand[0].def_id),
            "Strike"
        );
    }
    card_pair_test!(
    ...
    fn reckless_charge_inserts_dazed_with_card_random_without_shuffling_after_nonlethal_damage() {
        ...
        assert_eq!(
            empty
                .card_registry
                .card_name(empty.state.draw_pile[0].def_id),
            "Dazed"
        );
        ...
        assert!(lethal
            ...
            .draw_pile
            .iter()
            .all(|card| { lethal.card_registry.card_name(card.def_id) != "Dazed" }));
    }
    card_pair_test!(
        rupture,
        "Rupture",
    ...
    fn searing_blow_repeated_upgrades_preserve_counter_and_damage_curve() {
        ...
        let mut engine = engine_for(&[], &[], &[], vec![enemy_no_intent("JawWorm", 50, 50)], 2);
    ...
    fn exhume_excludes_itself_and_uses_java_singleton_selection_rules() {
        ...
        assert_eq!(
            singleton
                .card_registry
                .card_name(singleton.state.hand[0].def_id),
            "Strike"
        );
        ...
        assert_eq!(
            filtered_choice
                .card_registry
                .card_name(filtered_choice.state.hand[0].def_id),
            "Strike"
        );
    }
    card_pair_test!(
    ...
    fn juggernaut_source_uses_card_random_thorns_on_each_positive_block_gain() {
        ...
        for (card_id, damage) in [("Juggernaut", 5), ("Juggernaut+", 7)] {
            ...
            for (idx, enemy) in engine.state.enemies.iter().enumerate() {
                assert_eq!(
                    enemy.entity.hp,
                    40 - i32::from(idx == expected_target) * damage
                );
    ...
    fn dropkick_refunds_and_draws_when_its_target_was_vulnerable() {
        ...
        assert!(vulnerable
            ...
            .hand
            .iter()
            .any(|card| { vulnerable.card_registry.card_name(card.def_id) == "Defend" }));
    ...
    fn clash_requires_only_attacks() {
        ...
        let clash_idx = e
            ...
            .hand
            .iter()
            .position(|card| e.card_registry.card_name(card.def_id) == "Clash")
            .expect("Clash should be in hand");
        assert!(!e.get_legal_actions().iter().any(|action| matches!(
            action,
    ...
    fn iron_wave_source_blocks_before_damage_and_sharp_hide_retaliation() {
        ...
        for (card_id, amount, hp_loss, block_left) in
            [("Iron Wave", 5, 1, 0), ("Iron Wave+", 7, 0, 1)]
        {
            let mut engine = engine_for(
                &[card_id],
                &[],
    ...
    fn shrug_it_off_plus_gains_eleven_block_then_draws_exactly_one() {
        ...
        let mut engine = engine_for(&[], &[], &[], vec![enemy_no_intent("JawWorm", 50, 50)], 1);
    ...
    fn warcry_variants_draw_then_put_one_card_on_top_and_exhaust() {
        ...
        assert_eq!(
            e.card_registry.card_name(e.state.draw_pile[0].def_id),
            "Strike"
        );
        ...
        let choice = upgraded
            .choice
            .as_ref()
            .expect("Warcry+ should choose one hand card");
        ...
        assert_eq!(
            upgraded.card_registry.card_name(
                upgraded
                    .state
                    .draw_pile
                    .last()
                    .expect("selected card on draw")
                    .def_id
            ),
            selected_name,
        );
