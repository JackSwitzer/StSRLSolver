...
fn relic_wave8_counter_relics_track_hidden_runtime_state_without_helper_path() {
    ...
    fan.state.hand = make_deck_n("Strike_R", 3);
    fan.state.draw_pile.clear();
    fan.state.discard_pile.clear();

    assert!(play_on_enemy(&mut fan, "Strike_R", 0));
    assert!(play_on_enemy(&mut fan, "Strike_R", 0));
    ...
    nunchaku.state.hand = make_deck_n("Strike_R", 10);

    for _ in 0..9 {
        assert!(play_on_enemy(&mut nunchaku, "Strike_R", 0));
    }
    assert_eq!(
...
fn relic_wave8_pen_nib_orange_pellets_and_velvet_choker_use_runtime_path() {
    ...
    pellets.state.hand = make_deck(&["Strike_R", "Defend_R", "Inflame"]);
    pellets.state.player.set_status(sid::WEAKENED, 2);
    pellets.state.player.set_status(sid::VULNERABLE, 2);
    assert!(play_on_enemy(&mut pellets, "Strike_R", 0));
    assert!(play_self(&mut pellets, "Defend_R"));
    ...
    nib.state.hand = make_deck_n("Strike_R", 10);
    for _ in 0..9 {
        assert!(play_on_enemy(&mut nib, "Strike_R", 0));
    }
    ...
    choker.state.hand = make_deck_n("Defend_R", 7);
    for _ in 0..6 {
