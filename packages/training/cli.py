...
def _generate_phase1_corpus(
    *,
    ...
    requests = build_phase1_corpus_requests(plan, total_cases=target_cases)
    summary = _write_corpus_artifacts(output_dir, requests)
    _write_json(
...
def _collect_puct_targets(
    *,
    ...
    requests = _load_corpus_or_requests(input_path)
    summary = _collect_puct_targets_artifacts(
        requests,
        output_dir,
...
def _run_phase1_puct_overnight(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    plan = default_watcher_a0_act1_corpus_plan()
    corpus_requests = build_phase1_corpus_requests(plan, total_cases=target_cases)
    corpus_summary = _write_corpus_artifacts(output_dir, corpus_requests)
    collection_summary = _collect_puct_targets_artifacts(
        corpus_requests,
        output_dir,
        collection_passes=collection_passes,
    )
    ...
    seed_summary = _validate_seed_suite(output_dir=output_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate-seed-suite":
        summary = _validate_seed_suite(output_dir=Path(args.output_dir))
        print(json.dumps(summary, indent=2, sort_keys=True))
        return 0
