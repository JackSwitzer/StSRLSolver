...
from .benchmarking import BenchmarkFrontierPoint, build_frontier_report, build_phase1_puct_collection_report
...
from .seed_suite import default_watcher_validation_seed_suite, validate_watcher_validation_seed_suite
...
from .stage2_pipeline import (
    ...
    records_to_preference_examples,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    generate_corpus_parser = subparsers.add_parser(
        ...
        help="Generate the deterministic synthetic Watcher A0 Act 1 corpus",
    )
    generate_corpus_parser.add_argument("--output-dir", required=True, help="Directory for corpus artifacts")
    generate_corpus_parser.add_argument("--target-cases", type=int, default=PHASE1_CORPUS_TARGET_CASES)

    collect_targets_parser = subparsers.add_parser(
        "collect-puct-targets",
        help="Partition a generated corpus into multi-pass PUCT target bundles",
    )
    collect_targets_parser.add_argument("--input", required=True, help="Corpus JSONL generated by generate-phase1-corpus")
    collect_targets_parser.add_argument("--output-dir", required=True, help="Directory for PUCT target artifacts")
    ...
    train_checkpoint_parser = subparsers.add_parser(
        ...
        help="Train a checkpoint from collected PUCT target bundles",
    )
    train_checkpoint_parser.add_argument(
        "--input-dir",
        required=True,
        help="Directory containing puct_targets.jsonl or pass-specific bundles",
    )
    train_checkpoint_parser.add_argument("--output-dir", required=True, help="Directory for checkpoint and summary artifacts")
    train_checkpoint_parser.add_argument("--epochs", type=int, default=4)
    train_checkpoint_parser.add_argument("--learning-rate", type=float, default=0.05)
    train_checkpoint_parser.add_argument("--top-k", type=int, default=4)
    train_checkpoint_parser.add_argument("--backend", choices=("linear", "mlx"), default="linear")
    train_checkpoint_parser.add_argument("--checkpoint", help="Optional checkpoint to load before training")
    train_checkpoint_parser.add_argument("--no-update", action="store_true", help="Disable weight updates and run pure scoring")

    validate_seed_parser = subparsers.add_parser(
        "validate-seed-suite",
        help="Validate and report on the fixed 3-seed Watcher validation suite",
    )
    validate_seed_parser.add_argument("--output-dir", required=True, help="Directory for seed-suite report artifacts")
    validate_seed_parser.add_argument("--backend", choices=("linear", "mlx"), default="linear")
    validate_seed_parser.add_argument("--checkpoint", help="Optional checkpoint to score during seed validation")

    puct_overnight_parser = subparsers.add_parser(
        "run-phase1-puct-overnight",
        help="Generate corpus, collect PUCT targets, train a checkpoint, and validate seeds",
    )
    puct_overnight_parser.add_argument("--output-dir", required=True, help="Directory for phase-1 PUCT artifacts")
    puct_overnight_parser.add_argument("--target-cases", type=int, default=PHASE1_CORPUS_TARGET_CASES)
    ...
    puct_overnight_parser.add_argument("--learning-rate", type=float, default=0.05)
    puct_overnight_parser.add_argument("--top-k", type=int, default=4)
    puct_overnight_parser.add_argument("--backend", choices=("linear", "mlx"), default="linear")
...
def _load_jsonl_rows(path: Path) -> tuple[dict[str, Any], ...]:
    return tuple(_iter_jsonl(path))


def _load_prepared_corpus_requests(path: Path) -> tuple[PreparedCorpusRequest, ...]:
    return tuple(PreparedCorpusRequest.from_dict(row) for row in _load_jsonl_rows(path))


def _load_preference_examples(path: Path) -> list[CombatPreferenceExample]:
    return [CombatPreferenceExample.from_dict(row) for row in _load_jsonl_rows(path)]


def _write_prepared_corpus_requests(path: Path, requests: Iterable[PreparedCorpusRequest]) -> None:
    _write_jsonl(path, (request.to_dict() for request in requests))


def _write_preference_examples(path: Path, examples: Iterable[CombatPreferenceExample]) -> None:
    _write_jsonl(path, (example.to_dict() for example in examples))


def _corpus_examples_to_preferences(
    prepared_requests: Iterable[PreparedCorpusRequest],
    *,
    collection_passes: int,
) -> list[CombatPreferenceExample]:
    examples: list[CombatPreferenceExample] = []
    for prepared in prepared_requests:
        pass_index = prepared.corpus_index % collection_passes
        examples.append(
            CombatPreferenceExample(
                request=prepared.request,
                preferred_action_id=prepared.preferred_action_id,
                metadata={
                    "corpus_index": prepared.corpus_index,
                    "corpus_id": prepared.corpus_id,
                    "corpus_group": prepared.corpus_group,
...
def _load_corpus_or_requests(path: Path) -> tuple[PreparedCorpusRequest, ...]:
    if path.is_dir():
        corpus_path = path / "corpus.jsonl"
        if corpus_path.exists():
            return _load_prepared_corpus_requests(corpus_path)
        raise FileNotFoundError(f"missing corpus.jsonl in {path}")
    return _load_prepared_corpus_requests(path)
...
def _write_corpus_artifacts(
    ...
    _write_prepared_corpus_requests(output_dir / "corpus.jsonl", requests)
    summary = summarize_phase1_corpus_requests(requests)
    _write_json(output_dir / "corpus_summary.json", summary)
    return summary


def _collect_puct_targets_artifacts(
    requests: tuple[PreparedCorpusRequest, ...],
    output_dir: Path,
    *,
    collection_passes: int,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    examples = _corpus_examples_to_preferences(requests, collection_passes=collection_passes)
    pass_buckets: dict[int, list[CombatPreferenceExample]] = defaultdict(list)
    for prepared, example in zip(requests, examples):
        pass_index = prepared.corpus_index % collection_passes
        pass_buckets[pass_index].append(example)

    for pass_index in range(collection_passes):
        pass_examples = pass_buckets.get(pass_index, [])
        pass_path = output_dir / f"puct_targets_pass_{pass_index:03d}.jsonl"
        _write_preference_examples(pass_path, pass_examples)

    combined_path = output_dir / "puct_targets.jsonl"
    _write_preference_examples(combined_path, examples)
    report = build_phase1_puct_collection_report(requests, collection_passes=collection_passes)
    _write_json(output_dir / "puct_targets_report.json", report.to_dict())
    (output_dir / "puct_targets_report.md").write_text(report.to_markdown(), encoding="utf-8")
    return {
        "corpus_name": report.corpus_name,
        "total_cases": report.total_cases,
        "collection_passes": report.collection_passes,
        "pass_count": len(report.pass_summaries),
        "combined_targets": str(combined_path),
        "pass_targets": [
            str(output_dir / f"puct_targets_pass_{index:03d}.jsonl")
            for index in range(collection_passes)
        ],
    }


def _load_puct_target_examples(input_dir: Path) -> list[CombatPreferenceExample]:
    collection = input_dir / "puct_collection.jsonl"
    if collection.exists():
        records = [PuctCollectionRecord.from_dict(row) for row in _load_jsonl_rows(collection)]
        return records_to_preference_examples(records)
    combined = input_dir / "puct_targets.jsonl"
    if combined.exists():
        return _load_preference_examples(combined)
    examples: list[CombatPreferenceExample] = []
    for path in sorted(input_dir.glob("puct_targets_pass_*.jsonl")):
        examples.extend(_load_preference_examples(path))
    if examples:
        return examples
    if input_dir.is_file():
        return _load_preference_examples(input_dir)
    raise FileNotFoundError(f"no puct target bundles found in {input_dir}")


def _write_seed_validation_artifacts(output_dir: Path) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    report = validate_watcher_validation_seed_suite()
    _write_json(output_dir / "seed_validation_report.json", report.to_dict())
    (output_dir / "seed_validation_report.md").write_text(report.to_markdown(), encoding="utf-8")
    return {
...
def _generate_phase1_corpus(
    *,
    ...
    _write_json(
        ...
        {
            "command": "generate-phase1-corpus",
            **summary,
        },
    )
    return summary
...
def _collect_puct_targets(
    *,
    ...
    records = write_puct_collection(
        output_dir,
        cases=cases,
        backend=backend,
        collection_passes=collection_passes,
    )
    examples = records_to_preference_examples(records)
    _write_preference_examples(output_dir / "puct_targets.jsonl", examples)
    summary = {
        "corpus_name": "watcher_a0_act1_snapshot",
        "total_cases": len(cases),
        "collection_passes": collection_passes,
        "pass_count": len({record.collection_pass for record in records}),
        "combined_targets": str(output_dir / "puct_targets.jsonl"),
        "collection_records": str(output_dir / "puct_collection.jsonl"),
        "backend": backend,
    }
    _write_json(
        output_dir / "puct_targets_summary.json",
        {
            "command": "collect-puct-targets",
...
def _train_puct_checkpoint(
    *,
    ...
    examples = _load_puct_target_examples(input_dir)
    ...
    _write_json(
        output_dir / "puct_training_summary.json",
        {
            "command": "train-puct-checkpoint",
...
def _run_phase1_puct_overnight(
    *,
    ...
    _write_preference_examples(output_dir / "puct_targets.jsonl", records_to_preference_examples(records))
    collection_summary = {
        "corpus_name": "watcher_a0_act1_snapshot",
        "total_cases": len(cases),
        "collection_passes": collection_passes,
    ...
    training_summary = _train_puct_checkpoint(
    ...
    seed_summary = _validate_seed_suite(
        output_dir=output_dir,
        backend=backend,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "generate-phase1-corpus":
        summary = _generate_phase1_corpus(
    ...
    if args.command == "train-puct-checkpoint":
        summary = _train_puct_checkpoint(
    ...
    if args.command == "run-phase1-puct-overnight":
        summary = _run_phase1_puct_overnight(
            output_dir=Path(args.output_dir),
            target_cases=args.target_cases,
            collection_passes=args.collection_passes,
            epochs=args.epochs,
