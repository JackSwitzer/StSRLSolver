...
pure-sim stack. Current proof is 3,081 passing tests, zero ignored, native Java
...
read as current source descriptions. The v1 differ, Rust v2 action replay,
language-neutral oracle-state projection, and offline human-bundle adapter are
...
| Unit suite | 3,081 passing tests, zero ignored. | `./scripts/test_engine_rs.sh test --lib`; 219 Rust files under `src/tests`. | Tests alone do not replace the incomplete full-run Java corpus. |
