...
| D4 | Transient does not auto-fade | unverified | `enemies/act3.rs::roll_transient` | `Transient.java::getMove` | Java Transient has Shimmer power that decays + the monster fades after N turns; in our engine the solver appears to try to actually kill 999 HP and takes max damage. Surfaced as the only `failed` combat in the extended-catalog smoke. Needs investigation. | **open** |
