...
**Current level: B1 achieved, B2 awaiting reminted oracle data** (2026-07-20). The native RNG, run generation, canonical actions, checkpoints, and pure-core stack are locally green at 3,081 tests with zero ignored. The language-neutral v2 state projection and offline adapter process all 14 human bundles; the deepest comparable prefix is 17/607 before an omitted recorder action. See `docs/work_units/audit-reports/watcher-a0-oracle-closure.md`.
...
```bash
./scripts/test_engine_rs.sh test --lib     # 3081 green, includes oracle-state and recording-bundle tests
