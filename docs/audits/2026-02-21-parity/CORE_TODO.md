...
- Potions: `granular-potions.md` has `1` unchecked (Fairy invariants still open)
...
- [ ] `POT-002` Add deterministic RNG-counter assertions for Discovery/Snecko/Entropic/Distilled paths.
- [ ] `POT-003` Ensure all selection potions complete action roundtrip coverage (`missing params => candidate actions`).
- [ ] `POT-004` Close Fairy in a Bottle invariants (trigger, consumption, Sacred Bark % and defeat prevention).
