...
| relics | `packages/engine/state/run.py`, `packages/engine/registry/relics.py`, `packages/engine/game.py` | partial | Orrery + bottled selection-surface fixed; Dolly remains |
...
| rewards/shops/rest/map | `packages/engine/handlers/reward_handler.py`, `shop_handler.py`, `rooms.py`, `game.py` | partial | selection interception now covers Orrery + bottled; Dolly/event paths remain |
...
- `RunState._on_relic_obtained` still auto-selects for `DollysMirror` when unresolved.
...
- Suite result is green (`4616 passed, 5 skipped`), but some audit tests still document known bugs instead of enforcing parity behavior.
- Examples:
...
- [ ] Add action-surface tests for remaining relic acquisition selections (`REL-008` and follow-ups).
